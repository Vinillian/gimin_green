...
export const containerController = {
  moveToPress(containerIds) {
    ...
    const shelfSpace = store.shelves.map(shelf => ({ shelf, free: shelf.freeSpace }));
    const totalFree = shelfSpace.reduce((acc, s) => acc + s.free, 0);
    if (containers.length > totalFree) return false;
  ...
  moveToLight(containerIds) {
    ...
    if (containers.length > availableSpace) return false;
  ...
  spray(containerIds) {
    ...
    if (!store.resources.hasWater(totalCost)) return false;
